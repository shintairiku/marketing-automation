...
- v1 の更新正本にはしない
- 将来、ユーザー向け編集 UI やエクスポート、Markdown ベースの参照文脈などで `.md` 的に扱う可能性を残すため、DB カラム自体は維持する
...
- `company_memory_update(decision="update", fields=...)` を受けても、
  current JSON に `fields` を merge して canonicalize 後の JSON が現在の `content_json` と完全一致なら保存しない
...
system prompt に直接埋め込まず、`content_json` をサーバー側で次の 2 形式に整形して、
実行時の入力メッセージに差し込む。

- `会社共通メモ`: canonical JSON 全体
...
- 人向け要約と canonical JSON の二重表現を避け、更新時のキー解釈を一貫させやすい

### 注入テキストのルール

- `会社共通メモ` は canonical JSON 全体を code fence 付きで渡す
- 空欄フィールドも省略しない
- キー名は canonical schema のキー名をそのまま使う
- 余計な自然文整形や見出し分割は行わない
- 注入時には次の注意書きを前置する
  - 会社共通メモは補助文脈である
  - 矛盾がない限り会社・サイトの基本前提として活用する
  - 現在のユーザー指示と矛盾する場合はユーザー指示を優先する
  - WordPress の最新状態が必要な場合はツールで再確認する

### canonical JSON の渡し方

- `会社共通メモ` 自体を canonical JSON 全体として code fence 付きで渡す
- `company_memory_update(decision="update", fields=...)` を呼ぶ時は、この shape を基準に「変更したいフィールドだけ」を返させる
- canonical JSON は model に現在値の shape と埋まり具合を見せ、未指定フィールドを再生成させないために渡す
...
- 更新が必要なら、変更したいフィールドだけを `fields` に含めて `company_memory_update(decision="update", fields=...)` を呼ぶ
- ツール側で current JSON に対して field merge を行い、その後 schema validation / canonicalize / version 付き保存を行う
- update 判断時は、入力に含まれる `会社共通メモ` と `会社共通メモの空欄フィールド` を基準にする
...
- `fields`: 更新したいフィールドだけを含むオブジェクト（`decision="update"` の時のみ必須）
- `fields` に含めなかったキーは変更しない
- scalar フィールドも配列フィールドも、`fields` に指定した場合はそのフィールド全体を置換する
- v1 では `append` / `remove` のような配列専用操作は持たせない
- `version` は引数に含めない
...
ルール:
- current JSON に `fields` を merge し、canonicalize 後に current と完全一致なら `no_change`
- 保存成功時のみ `saved`
...
- `update` の場合は `fields` のキーを検証し、current JSON に merge する
...
- `fields` 未指定で `decision="update"` を受け付けない
- schema にないキーを保存しない
- LLM の返した `fields` を無検証でそのまま保存しない
- 単発記事専用の事情を保存しない
- 一時的なキャンペーン情報を保存しない
- 実在記事 URL 一覧を保存しない
...
- 未指定フィールドは変更しない
- scalar フィールドは指定値で置換する
- 配列フィールドも指定値で丸ごと置換する
- v1 では配列への部分追加・部分削除は行わない
- `fields` 適用後は全体を canonicalize し、空要素除去・重複除去・default 補完を行う
...
- 将来 `append` / `remove` のような配列操作を追加する可能性はあるが、v1 では採用しない
...
- `render_company_memory_text(content_json)` または同等の JSON 表示関数
...
v1 実装時は、company memory 更新の挙動確認のため、`company_memory_update` 実行後にバックエンドログへ更新対象 `fields` と canonicalize 後の `content_json` 全文を出力してよい。
...
- 取得できた `content_json` を canonical JSON 表示へ整形して入力へ注入する
