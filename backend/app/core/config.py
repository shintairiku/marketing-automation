...
class Settings(BaseSettings):
    ...
    memory_embed_model: str = Field(default_factory=lambda: os.getenv("MEMORY_EMBED_MODEL", "text-embedding-3-small"))
    memory_embed_dim: int = Field(default_factory=lambda: int(os.getenv("MEMORY_EMBED_DIM", "1536")))
    memory_embed_batch_size: int = Field(default_factory=lambda: int(os.getenv("MEMORY_EMBED_BATCH_SIZE", "100")))
    memory_embed_max_retries: int = Field(default_factory=lambda: int(os.getenv("MEMORY_EMBED_MAX_RETRIES", "5")))
    memory_embed_retry_base_sec: int = Field(default_factory=lambda: int(os.getenv("MEMORY_EMBED_RETRY_BASE_SEC", "1")))
