...
try:
    ...
    from app.infrastructure.logging.agents_logging_integration import (
        MultiAgentWorkflowLogger,
    )
    from app.infrastructure.external_apis.notion_service import (
        NotionService as NotionSyncService,
    )
...
TRANSITION_STEPS = {"persona_selected", "theme_selected", "research_plan_approved"}

TERMINAL_STEPS = {"completed", "error"}
...
class GenerationFlowManager:
    ...
    async def run_generation_loop(
        ...
        try:
            while context.current_step not in ["completed", "error"]:
                console.print(
                    f"[green]生成ループ開始: {context.current_step} (process_id: {process_id})[/green]"
                )
                ...
                console.rule(
                    f"[bold yellow]API Step: {context.current_step}[/bold yellow]"
                )
                ...
                if process_id and user_id and context.current_step != previous_step:
                    logger.info(
                        f"🔍 Step changed from '{previous_step}' to '{context.current_step}', saving snapshot for '{previous_step}'"
                    )
                    ...
                    logger.debug(
                        f"⏭️ No snapshot: process_id={process_id}, user_id={user_id}, prev={previous_step}, curr={context.current_step}"
                    )

        except asyncio.CancelledError:
            console.print("[yellow]Generation loop cancelled.[/yellow]")
            await self.service.utils.send_error(
                context, "Generation process cancelled.", context.current_step
            )
        except Exception as e:
    ...
    async def execute_step(
        ...
        if context.current_step == "start":
            ...
            await self.handle_keyword_analyzing_step(
                context, run_config, process_id, user_id
            )
        elif context.current_step == "persona_generating":
            await self.handle_persona_generating_step(
                context, run_config, process_id, user_id
            )
            ...
            await self.handle_theme_generating_step(
                context, run_config, process_id, user_id
            )
            ...
            await self.handle_research_planning_step(
                context, run_config, process_id, user_id
            )
            ...
            await self.handle_research_synthesizing_step(
                context, run_config, process_id, user_id
            )
        elif context.current_step == "outline_generating":
            await self.handle_outline_generating_step(
                context, run_config, process_id, user_id
            )
            ...
            await self.handle_writing_sections_step(
                context, run_config, process_id, user_id
            )
    ...
    async def handle_keyword_analyzing_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if context.executing_step == "keyword_analyzing":
            console.print(
                "[yellow]キーワード分析は既に実行中です。スキップします。[/yellow]"
            )
        ...
        agent_output = await self.run_agent(
            current_agent, agent_input, context, run_config
        )
        ...
        if isinstance(agent_output, SerpKeywordAnalysisReport):
            ...
            await self.save_and_send_keyword_analysis(
                context, agent_output, process_id, user_id
            )
            ...
            await self.service.utils.send_error(
                context,
                f"SerpAPIキーワード分析中に予期しないエージェント出力タイプ ({type(agent_output)}) を受け取りました。",
            )
            context.current_step = "error"

    async def handle_persona_generating_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if isinstance(agent_output, GeneratedPersonasResponse):
            context.generated_detailed_personas = [
                p.description for p in agent_output.personas
            ]
            context.current_step = "persona_generated"
            console.print(
                f"[cyan]{len(context.generated_detailed_personas)}件の具体的なペルソナを生成しました。クライアントの選択を待ちます...[/cyan]"
            )
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.error(
                        f"Failed to save context after persona generation: {save_err}"
                    )

            await self.handle_persona_user_interaction(context, process_id, user_id)
        else:
            console.print(
                "[red]ペルソナ生成中に予期しないエージェント出力タイプを受け取りました。[/red]"
            )
            context.current_step = "error"

    async def handle_persona_user_interaction(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        if process_id and user_id:
            try:
                from app.domains.seo_article.services.flow_service import (
                    get_supabase_client,
                )
                ...
                await self.service.persistence_service.update_process_state(
                    process_id=process_id, current_step_name="persona_generated"
                )
                ...
                await supabase.rpc(
                    "mark_process_waiting_for_input",
                    {
                        ...
                        "p_input_type": "select_persona",
                        "p_timeout_minutes": 60,
                    },
                ).execute()

                logger.info(
                    "Process state marked waiting for persona selection with RPC"
                )
            except Exception as save_err:
                logger.error(
                    f"Failed to mark process waiting for persona selection: {save_err}"
                )
        ...
        if user_response_message:
            ...
            payload = self.service.utils.convert_payload_to_model(
                payload_dict, response_type
            )

            if (
                response_type == UserInputType.SELECT_PERSONA
                and payload
                and isinstance(payload, SelectPersonaPayload)
            ):
                await self.handle_persona_selection(
                    context, payload, process_id, user_id
                )
            elif response_type == UserInputType.REGENERATE:
                await self.handle_persona_regeneration(context)
            elif (
                response_type == UserInputType.EDIT_AND_PROCEED
                and payload
                and isinstance(payload, EditAndProceedPayload)
            ):
                await self.handle_persona_edit(context, payload, process_id, user_id)
            else:
                await self.handle_invalid_persona_response(
                    context, response_type, payload
                )

    async def handle_persona_selection(
        self,
        context: ArticleContext,
        payload: SelectPersonaPayload,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
    ):
        """ペルソナ選択の処理"""
        ...
        if 0 <= selected_id < len(context.generated_detailed_personas):
            context.selected_detailed_persona = context.generated_detailed_personas[
                selected_id
            ]
            context.current_step = "persona_selected"
            console.print(
                f"[green]クライアントがペルソナID {selected_id} を選択しました。[/green]"
            )
            await self.service.utils.send_server_event(
                context,
                StatusUpdatePayload(
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.error(
                        f"Failed to save context after persona selection: {save_err}"
                    )
        else:
    ...
    async def handle_persona_edit(
        self,
        context: ArticleContext,
        payload: EditAndProceedPayload,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
    ):
        """ペルソナ編集の処理"""
        ...
        if edited_persona_description and isinstance(edited_persona_description, str):
            ...
            context.current_step = "persona_selected"
            console.print(
                f"[green]クライアントがペルソナを編集し、選択しました: {context.selected_detailed_persona[:50]}...[/green]"
            )
            await self.service.utils.send_server_event(
                context,
                StatusUpdatePayload(
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.info(
                        "Context saved successfully after persona editing and selection"
                    )
                except Exception as save_err:
                    logger.error(
                        f"Failed to save context after persona editing: {save_err}"
                    )
        else:
            await self.service.utils.send_error(
                context, "Invalid edited persona content."
            )
            context.current_step = "persona_generated"

    async def handle_invalid_persona_response(
        self, context: ArticleContext, response_type: UserInputType, payload
    ):
        """無効なペルソナ応答の処理"""
        payload_type = type(payload).__name__ if payload else "None"
        await self.service.utils.send_error(
            context,
            f"予期しない応答 ({response_type}, {payload_type}) がペルソナ選択で受信されました。",
        )
        context.current_step = "persona_generated"

    async def handle_persona_selected_step(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        if process_id and user_id:
            try:
                ...
                logger.error(
                    f"Failed to save context after persona selection step: {save_err}"
                )
    ...
    async def handle_theme_generating_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if not context.selected_detailed_persona:
            await self.service.utils.send_error(
                context,
                "詳細ペルソナが選択されていません。テーマ生成をスキップします。",
                "theme_generating",
            )
            context.current_step = "error"
            return
        ...
        if process_id and user_id:
            try:
                ...
                logger.error(
                    f"Failed to save context at theme generation start: {save_err}"
                )
        ...
        if isinstance(agent_output, ThemeProposal):
            await self.handle_theme_proposal_result(
                context, agent_output, process_id, user_id
            )
        elif isinstance(agent_output, ClarificationNeeded):
            await self.service.utils.send_error(
                context, f"テーマ生成で明確化が必要です: {agent_output.message}"
            )
            context.current_step = "error"
        else:
            await self.service.utils.send_error(
                context,
                f"テーマ生成中に予期しないエージェント出力タイプ ({type(agent_output)}) を受け取りました。",
            )
            context.current_step = "error"
    ...
    async def handle_theme_proposal_result(
        self,
        context: ArticleContext,
        agent_output: ThemeProposal,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
    ):
        """テーマ提案結果の処理"""
        ...
        if context.generated_themes:
            ...
            console.print(
                f"[cyan]{len(context.generated_themes)}件のテーマ案を生成しました。クライアントの選択を待ちます...[/cyan]"
            )
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.error(
                        f"Failed to save context after theme generation: {save_err}"
                    )
            ...
            await self.service.utils.send_error(
                context,
                "テーマ案がエージェントによって生成されませんでした。再試行します。",
            )
            context.current_step = "theme_generating"

    async def handle_theme_user_interaction(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        themes_data_for_client = [
            ThemeProposalData(
                title=idea.title, description=idea.description, keywords=idea.keywords
            )
        ...
        if process_id and user_id:
            try:
                ...
                context.current_step = "theme_proposed"
                await self.service.persistence_service.update_process_state(
                    process_id=process_id, current_step_name="theme_proposed"
                )
                ...
                logger.error(
                    f"Failed to mark process waiting for theme selection: {save_err}"
                )
        ...
        if user_response_message:
            ...
            if (
                response_type == UserInputType.SELECT_THEME
                and payload
                and isinstance(payload, SelectThemePayload)
            ):
                ...
                await self.handle_invalid_theme_response(
                    context, response_type, payload
                )
    ...
    async def handle_theme_selection(
        ...
        if 0 <= selected_index < len(context.generated_themes):
            ...
            console.print(
                f"[green]クライアントがテーマ「{context.selected_theme.title}」を選択しました。[/green]"
            )
            await self.service.utils.send_server_event(
                context,
                StatusUpdatePayload(
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.error(
                        f"Failed to save context after theme selection: {save_err}"
                    )

            console.print(
                f"[blue]テーマ選択処理完了。次のステップ: {context.current_step}[/blue]"
            )
        else:
            await self.service.utils.send_error(
                context, f"無効なテーマインデックスが選択されました: {selected_index}"
            )
            context.current_step = "theme_proposed"
    ...
    async def handle_theme_edit(
        ...
        try:
            ...
            if (
                isinstance(edited_theme_data.get("title"), str)
                and isinstance(edited_theme_data.get("description"), str)
                and isinstance(edited_theme_data.get("keywords"), list)
            ):
                context.selected_theme = ThemeIdea(**edited_theme_data)
                context.reset_after_theme_selection()
                context.current_step = "theme_selected"
                console.print(
                    f"[green]クライアントがテーマを編集し、選択しました: {context.selected_theme.title}[/green]"
                )
                await self.service.utils.send_server_event(
                    context,
                    StatusUpdatePayload(
                ...
                if process_id and user_id:
                    try:
                        ...
                        logger.info(
                            "Context saved successfully after theme editing and selection"
                        )
                    except Exception as save_err:
                        logger.error(
                            f"Failed to save context after theme editing: {save_err}"
                        )
            else:
                await self.service.utils.send_error(
                    context, "Invalid edited theme content structure."
                )
                context.current_step = "theme_proposed"
        except (ValidationError, TypeError, AttributeError) as e:
            await self.service.utils.send_error(
                context, f"Error processing edited theme: {e}"
            )
            context.current_step = "theme_proposed"

    async def handle_invalid_theme_response(
        self, context: ArticleContext, response_type: UserInputType, payload
    ):
        """無効なテーマ応答の処理"""
        payload_type = type(payload).__name__ if payload else "None"
        await self.service.utils.send_error(
            context,
            f"予期しない応答 ({response_type}, {payload_type}) がテーマ選択で受信されました。",
        )
        context.current_step = "theme_proposed"

    async def handle_theme_selected_step(
        self, context: ArticleContext, process_id: Optional[str] = None
    ):
        """テーマ選択完了ステップの処理"""
        console.print(
            f"[blue]theme_selectedステップを処理中... (process_id: {process_id})[/blue]"
        )
        ...
        if is_outline_first:
            ...
            console.print(
                "[blue]theme_selectedからoutline_generatingに遷移します...[/blue]"
            )
        else:
        ...
        console.print(
            f"[blue]{context.current_step}ステップに移行完了。継続中... (process_id: {process_id})[/blue]"
        )

    async def handle_research_planning_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        # 現行フローではこの処理を通りません。
        console.print(
            f"[blue]research_planningステップを開始します。selected_theme: {context.selected_theme.title if context.selected_theme else 'None'}[/blue]"
        )
        if not context.selected_theme:
            console.print(
                "[red]テーマが選択されていません。リサーチ計画作成をスキップします。[/red]"
            )
            context.current_step = "error"
            return
        ...
        if isinstance(agent_output, ResearchPlan):
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.info(
                        "Context saved successfully after research plan generation"
                    )
                except Exception as save_err:
                    logger.error(
                        f"Failed to save context after research plan generation: {save_err}"
                    )
        elif isinstance(agent_output, ClarificationNeeded):
            await self.service.utils.send_error(
                context, f"リサーチ計画作成で明確化が必要です: {agent_output.message}"
            )
            context.current_step = "error"
        else:
            await self.service.utils.send_error(
                context,
                f"リサーチ計画作成中に予期しないエージェント出力タイプ ({type(agent_output)}) を受け取りました。",
            )
            context.current_step = "error"

    async def handle_user_input_step_or_unknown(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        if context.current_step in USER_INPUT_STEPS:
            console.print(
                f"[yellow]ステップ {context.current_step} はユーザー入力が必要です。ユーザー応答を処理します。[/yellow]"
            )
    ...
    async def execute_single_step(
        ...
        console.rule(
            f"[bold yellow]Background Step: {context.current_step}[/bold yellow]"
        )

        # バックグラウンド専用のステップ実行
        await self.execute_background_step(context, run_config, process_id, user_id)

    async def execute_background_step(
        self,
        context: "ArticleContext",
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if context.current_step == "start":
            ...
            if context.current_step in USER_INPUT_STEPS:
                console.print(
                    f"[yellow]ステップ {context.current_step} はユーザー入力が必要です。バックグラウンド処理を一時停止。[/yellow]"
                )
                return
            else:
                console.print(
                    f"[red]未定義または処理不可能なステップ: {context.current_step}[/red]"
                )
                context.current_step = "error"

    async def execute_research_planning_background(
        self, context: "ArticleContext", run_config: RunConfig
    ):
        """リサーチ計画のバックグラウンド実行"""
        ...
        if isinstance(agent_output, ResearchPlan):
            ...
            console.print(
                "[red]リサーチ計画生成中に予期しないエージェント出力タイプを受け取りました。[/red]"
            )
            context.current_step = "error"

    async def execute_researching_background(
        self, context: "ArticleContext", run_config: RunConfig
    ):
        ...
        if not context.research_plan:
            console.print(
                "[red]承認されたリサーチ計画がありません。リサーチをスキップします。[/red]"
            )
            context.current_step = "error"
            return
        ...
        console.print(
            f"[cyan]🚀 {total_queries}件のリサーチクエリを並列実行開始...[/cyan]"
        )
        ...
        async def execute_single_query(query, query_index: int):
            ...
            try:
                console.print(
                    f"🔍 リサーチクエリ {query_index + 1}/{total_queries}: {query.query}"
                )
                ...
                if isinstance(agent_output, ResearchQueryResult):
                    console.print(
                        f"[green]✅ クエリ {query_index + 1} のリサーチが完了しました。[/green]"
                    )
                    return query_index, agent_output, True
                else:
                    console.print(
                        f"[red]❌ リサーチクエリ {query_index + 1} で予期しないエージェント出力タイプを受け取りました。[/red]"
                    )
                    return query_index, None, False

            except Exception as e:
                console.print(
                    f"[red]❌ リサーチクエリ {query_index + 1} でエラーが発生: {e}[/red]"
                )
        ...
        console.print(
            f"[cyan]🎉 並列リサーチ完了: {successful_queries}/{total_queries} 成功[/cyan]"
        )
    ...
    async def ensure_serp_analysis_fields(
        self, agent_output: SerpKeywordAnalysisReport, context: ArticleContext
    ):
    ...
    async def save_and_send_keyword_analysis(
        self,
        context: ArticleContext,
        agent_output: SerpKeywordAnalysisReport,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
    ):
        """キーワード分析結果の保存と送信"""
        ...
        if process_id and user_id:
            try:
                ...
                logger.info(
                    "Context saved successfully after keyword analysis completion"
                )
            except Exception as save_err:
                logger.error(
                    f"Failed to save context after keyword analysis: {save_err}"
                )
        ...
        if not context.target_length:
            ...
            console.print(
                f"[cyan]推奨目標文字数を設定しました: {context.target_length}文字[/cyan]"
            )
    ...
    async def run_agent(
        ...
        if context.current_step == "writing_sections" or getattr(agent, "name", "") in [
            "SectionWriterAgent",
            "SectionWriterWithImagesAgent",
        ]:
        ...
        console.print(
            f"[dim]Agent execution - process_id: {process_id}, workflow_loggers keys: {list(self.service.workflow_loggers.keys())}[/dim]"
        )

        # ワークフローロガーの取得・作成を確実に行う
        workflow_logger = (
            self.service.workflow_loggers.get(process_id) if process_id else None
        )
        if (
            not workflow_logger
            and process_id
            and LOGGING_ENABLED
            and MultiAgentWorkflowLogger
        ):
            console.print(
                f"[yellow]⚠️ No workflow logger found for process {process_id}, creating one now[/yellow]"
            )
            try:
                await self.ensure_workflow_logger(
                    context, process_id, getattr(context, "user_id", "unknown")
                )
                workflow_logger = self.service.workflow_loggers.get(process_id)
                console.print(
                    f"[green]✅ Successfully created workflow logger for process {process_id}[/green]"
                )
            except Exception as e:
                console.print(
                    f"[red]❌ Failed to create workflow logger for process {process_id}: {e}[/red]"
                )

        # エージェント実行のメインロジック
        with safe_custom_span(
            "agent_execution",
            data={
            ...
            for attempt in range(settings.max_retries):
                try:
                    console.print(
                        f"[dim]エージェント {agent.name} 実行開始 (試行 {attempt + 1}/{settings.max_retries})...[/dim]"
                    )
                    result = None
                    ...
                    if use_litellm:
                        ...
                        if is_image_mode:
                            output = ArticleSectionWithImages(
                                ...
                                or (
                                    context.generated_outline.sections[
                                        section_index
                                    ].heading
                    ...
                    if self._is_non_empty_output(output):
                        ...
                        logger.info(
                            f"エージェント {agent.name} 実行成功: {execution_time:.2f}秒, 試行回数: {attempt + 1}"
                        )
                        ...
                        # 出力の検証と変換
                        return await self.validate_and_convert_agent_output(
                            agent, output
                        )
                    else:
                        console.print(
                            f"[yellow]エージェント {agent.name} から有効な出力が得られませんでした。[/yellow]"
                        )
                        raise ModelBehaviorError(
                            f"No valid final output from agent {agent.name}"
                        )

                except Exception as e:
                    last_exception = e
                    await self.handle_agent_execution_error(
                        agent, e, attempt, start_time, workflow_logger
                    )

                    if (
                        self.should_break_retry(e)
                        or attempt >= settings.max_retries - 1
                    ):
                        break
            ...
            if last_exception:
                ...
                logger.error(
                    f"エージェント {agent.name} の実行に失敗しました（リトライ上限到達）: 総実行時間 {total_time:.2f}秒, 最終エラー: {type(last_exception).__name__}"
                )
                console.print(
                    f"[bold red]エージェント {agent.name} の実行に失敗しました（リトライ上限到達）。[/bold red]"
                )
                raise last_exception

            total_time = time.time() - start_time
            logger.error(
                f"エージェント {agent.name} execution finished unexpectedly: 総実行時間 {total_time:.2f}秒"
            )
    ...
    async def _run_section_with_litellm(
        ...
        *,
        ...
        if isinstance(input_data, list):
            history_messages = [
                _normalize_message(m) for m in input_data if m is not None
            ]
        else:
        ...
        if is_image_mode:
            try:
                ...
                final_html = (
                    parsed.get("content") or parsed.get("html_content") or accumulated
                )
                section_order = parsed.get("order") or section_order
                phs = (
                    parsed.get("images")
                    or parsed.get("image_placeholders")
                    or parsed.get("placeholders")
                    or []
                )
                ...
                console.print(
                    f"[yellow]画像モードのJSONパースに失敗しました: {parse_err}[/yellow]"
                )
                raise ModelBehaviorError(
                    f"Failed to parse image-mode JSON output: {parse_err}"
                ) from parse_err
    ...
    def _extract_json_from_text(self, text: str) -> Dict[str, Any]:
        ...
        if end == -1:
            raise json.JSONDecodeError(
                "No matching closing brace for JSON object", cleaned, start
            )
    ...
    async def validate_and_convert_agent_output(self, agent: Agent, output: Any) -> Any:
        ...
        if isinstance(
            ...
            try:
                ...
                output_model_map = {
                    "theme_proposal": ThemeProposal,
                    "outline": Outline,
                    "revised_article": RevisedArticle,
                    "clarification_needed": ClarificationNeeded,
                    "status_update": StatusUpdate,
                    "research_plan": ResearchPlan,
                    "research_query_result": ResearchQueryResult,
                ...
                if status_val in output_model_map:
                    ...
                    console.print(
                        f"[yellow]警告: 不明なstatus '{status_val}' を含むJSON応答。[/yellow]"
                    )
                    return parsed_output
            except (json.JSONDecodeError, ValidationError) as parse_error:
                console.print(
                    f"[yellow]警告: Agent応答のJSONパース/バリデーション失敗。内容: {output[:100]}... エラー: {parse_error}[/yellow]"
                )
                raise ModelBehaviorError(
                    f"Failed to parse/validate agent output: {parse_error}"
                ) from parse_error
        else:
            console.print(
                f"[yellow]警告: Agent応答が予期した型でない。型: {type(output)}[/yellow]"
            )
            raise ModelBehaviorError(
    ...
    async def log_agent_execution(
        self, workflow_logger, agent: Agent, result, execution_time: float, attempt: int
    ):
        """エージェント実行のログ記録"""
        if LOGGING_ENABLED and workflow_logger and self.service.logging_service:
            try:
                ...
                console.print(
                    f"[cyan]📋 Agent execution logged for {agent.name} (attempt {attempt + 1})[/cyan]"
                )
    ...
    async def handle_agent_execution_error(
        ...
        logger.warning(
            f"エージェント {agent.name} 実行エラー (試行 {attempt + 1}/{settings.max_retries}): {error_type} - {e}, 経過時間: {attempt_time:.2f}秒"
        )
        console.print(
            f"[yellow]エージェント {agent.name} 実行中にエラー (試行 {attempt + 1}/{settings.max_retries}): {error_type} - {e}[/yellow]"
        )

    async def handle_generation_error(
        self, context: ArticleContext, e: Exception, process_id: Optional[str] = None
    ):
        """生成プロセスのエラー処理"""
        context.current_step = "error"
        error_message = (
            f"記事生成プロセス中にエラーが発生しました: {type(e).__name__} - {str(e)}"
        )
        ...
        await self.service.utils.send_error(
            context, error_message, context.current_step
        )

    async def finalize_generation_loop(
        self, context: ArticleContext, process_id: Optional[str] = None
    ):
        """生成ループの最終化処理"""
        if LOGGING_ENABLED and process_id in self.service.workflow_loggers:
            try:
                ...
                # 切断耐性ステップかどうかを確認
                is_disconnection_resilient = (
                    context.current_step in DISCONNECTION_RESILIENT_STEPS
                )

                # セッション状態を決定
                session_status = (
                    "completed" if context.current_step == "completed" else "failed"
                )

                if (
                    is_disconnection_resilient
                    and context.current_step != "completed"
                    and context.current_step != "error"
                ):
                    # 切断耐性ステップでは、まだ処理が継続する可能性があるためログセッションを維持
                    console.print(
                        f"[cyan]Keeping log session active for disconnection-resilient step: {context.current_step}[/cyan]"
                    )
                    # ワークフロステップをログに記録
                    workflow_logger.log_workflow_step(
                        f"background_processing_{context.current_step}",
                        {
                            "step": context.current_step,
                    ...
                    console.print(
                        f"[cyan]Finalized log session for process {process_id} with status: {session_status}[/cyan]"
                    )

                    # Notionに自動同期（完了したセッションのみ）
                    if (
                        NOTION_SYNC_ENABLED
                        and self.service.notion_sync_service
                        and session_status == "completed"
                    ):
                        try:
                            console.print(
                                f"[yellow]🔄 Notionに自動同期開始: {process_id}[/yellow]"
                            )
                            if hasattr(
                                self.service.notion_sync_service,
                                "sync_session_to_notion",
                            ):
                                sync_success = self.service.notion_sync_service.sync_session_to_notion(
                                    workflow_logger.session_id
                                )
                                if sync_success:
                                    console.print(
                                        f"[green]✅ Notion自動同期完了: {process_id}[/green]"
                                    )
                                else:
                                    console.print(
                                        f"[red]❌ Notion自動同期失敗: {process_id}[/red]"
                                    )
                            else:
                                console.print(
                                    "[yellow]⚠️ Notion同期メソッドが利用できません。スキップします。[/yellow]"
                                )
                        except Exception as sync_err:
                            logger.warning(f"Notion auto-sync failed: {sync_err}")
                            console.print(
                                f"[red]❌ Notion自動同期エラー: {sync_err}[/red]"
                            )

                    # クリーンアップ
                    del self.service.workflow_loggers[process_id]
                    console.print(
                        f"[cyan]Workflow logger cleaned up for process {process_id}[/cyan]"
                    )
    ...
    async def send_final_status_message(self, context: ArticleContext):
        ...
        if context.current_step == "completed":
            ...
            console.print(
                f"[yellow]Generation loop stopped at user input step: {context.current_step}[/yellow]"
            )
        else:
            await self.service.utils.send_server_event(
                context,
    ...
    async def ensure_workflow_logger(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        if not process_id or not LOGGING_ENABLED:
            console.print(
                f"[yellow]Workflow logger not needed: process_id={process_id}, LOGGING_ENABLED={LOGGING_ENABLED}[/yellow]"
            )
            return
        ...
        if workflow_logger:
            console.print(
                f"[green]✅ Workflow logger already exists for process {process_id} (session: {workflow_logger.session_id})[/green]"
            )
            return

        # ワークフローロガーを作成
        console.print(
            f"[yellow]🔄 Creating workflow logger for process {process_id}[/yellow]"
        )
        try:
            # コンテキストから設定を構築
            ...
            console.print(
                f"[green]✅ Workflow logger stored for process {process_id}[/green]"
            )

        except Exception as e:
            logger.error(
                f"Failed to ensure workflow logger for process {process_id}: {e}"
            )
    ...
    def build_initial_config(self, context: ArticleContext) -> Dict[str, Any]:
        ...
        return {
            "initial_input": {
                ...
                "persona_type": context.persona_type.value
                if hasattr(context, "persona_type") and context.persona_type
                else None,
                "target_age_group": context.target_age_group.value
                if hasattr(context, "target_age_group") and context.target_age_group
                else None,
    ...
    async def restore_or_create_log_session(self, workflow_logger, process_id: str):
        ...
        if existing_session.data:
            ...
            steps_count = (
                supabase.table("workflow_step_logs")
                .select("step_order")
                .eq("session_id", workflow_logger.session_id)
                .execute()
            )
            if steps_count.data:
            ...
            console.print(
                f"[cyan]✅ Restored log session {workflow_logger.session_id} for process {process_id} (step: {workflow_logger.current_step})[/cyan]"
            )
        else:
            # 新しいログセッションを作成
            log_session_id = workflow_logger.initialize_session()
            console.print(
                f"[cyan]✅ Created new log session {log_session_id} for process {process_id}[/cyan]"
            )

    async def restore_logging_session(
        self, context: ArticleContext, process_id: str, user_id: str
    ):
        """ログセッションを復元または作成（復帰時用）"""
        console.print(
            f"[debug]Process restoration - LOGGING_ENABLED: {LOGGING_ENABLED}, MultiAgentWorkflowLogger: {MultiAgentWorkflowLogger is not None}[/debug]"
        )
        console.print(
            f"[debug]Current workflow_loggers keys: {list(self.service.workflow_loggers.keys())}[/debug]"
        )
        console.print(
            f"[debug]process_id {process_id} in workflow_loggers: {process_id in self.service.workflow_loggers}[/debug]"
        )

        if LOGGING_ENABLED and MultiAgentWorkflowLogger:
            try:
                console.print(
                    f"[debug]Checking workflow logger for process {process_id}. Current loggers: {list(self.service.workflow_loggers.keys())}[/debug]"
                )
                if process_id not in self.service.workflow_loggers:
                    console.print(
                        f"[green]Creating new workflow logger for restored process {process_id} with user_id {user_id}[/green]"
                    )
                    console.print(
                        f"[debug]LoggingService available: {self.service.logging_service is not None}[/debug]"
                    )
                    ...
                    console.print(
                        f"[debug]Creating MultiAgentWorkflowLogger with config: {initial_config}[/debug]"
                    )
                    workflow_logger = MultiAgentWorkflowLogger(
                        article_uuid=process_id,
                        user_id=user_id,
                    ...
                    console.print(
                        f"[debug]MultiAgentWorkflowLogger created. Has logging_service: {workflow_logger.logging_service is not None}[/debug]"
                    )
                    ...
                    if existing_session.data:
                        ...
                        workflow_logger.session_id = existing_session.data[0]["id"]
                        console.print(
                            f"[cyan]Found existing log session {workflow_logger.session_id} for process {process_id}[/cyan]"
                        )
                    else:
                        # 新しいログセッションを作成
                        try:
                            log_session_id = workflow_logger.initialize_session()
                            console.print(
                                f"[cyan]Created new log session {log_session_id} for restored process {process_id}[/cyan]"
                            )
                        except Exception as init_err:
                            logger.error(
                                f"Failed to initialize log session: {init_err}"
                            )
                            console.print(
                                f"[red]Failed to initialize log session: {init_err}[/red]"
                            )

                    self.service.workflow_loggers[process_id] = workflow_logger
                    console.print(
                        f"[green]Workflow logger for process {process_id} stored successfully[/green]"
                    )
            except Exception as e:
                logger.error(f"Failed to restore logging session: {e}")

    async def initialize_logging_session(
        self, context: ArticleContext, process_id: str, user_id: str, request
    ):
        """新規プロセス用のログセッション初期化"""
        console.print(
            f"[debug]LOGGING_ENABLED: {LOGGING_ENABLED}, MultiAgentWorkflowLogger: {MultiAgentWorkflowLogger is not None}[/debug]"
        )
        if LOGGING_ENABLED and MultiAgentWorkflowLogger:
            try:
                console.print(
                    f"[green]Creating workflow logger for process {process_id} with user_id {user_id}[/green]"
                )
                console.print(
                    f"[debug]Creating workflow logger for new process {process_id}[/debug]"
                )
                workflow_logger = MultiAgentWorkflowLogger(
                    article_uuid=process_id,
                    user_id=user_id,
                    ...
                    initial_config={
                        ...
                        "article_style_info": getattr(
                            context, "style_template_settings", {}
                        ),
                        "generation_theme_count": request.num_theme_proposals,
                        "target_age_group": request.target_age_group.value
                        if request.target_age_group
                        else None,
                        "persona_settings": {
                ...
                try:
                    log_session_id = workflow_logger.initialize_session()
                    console.print(
                        f"[cyan]Initialized log session {log_session_id} for process {process_id}[/cyan]"
                    )
                except Exception as init_err:
                    logger.error(
                        f"Failed to initialize log session for new process: {init_err}"
                    )
                    console.print(
                        f"[red]Failed to initialize log session: {init_err}[/red]"
                    )
                ...
                self.service.workflow_loggers[process_id] = workflow_logger
                console.print(
                    f"[green]Workflow logger stored in self.workflow_loggers[{process_id}][/green]"
                )
            except Exception as e:
                logger.error(f"Failed to initialize logging session: {e}")

    async def log_workflow_step(
        self, context: ArticleContext, step_name: str, step_data: Dict[str, Any] = None
    ):
        """ワークフローステップをログに記録"""
        ...
        try:
            ...
            if workflow_logger and self.service.logging_service:
                ...
                console.print(
                    f"[cyan]📊 Workflow step logged: {step_id} ({step_name})[/cyan]"
                )
    ...
    async def finalize_workflow_logger(
        self, process_id: str, status: str = "completed"
    ):
        ...
        if LOGGING_ENABLED and process_id in self.service.workflow_loggers:
            try:
                ...
                console.print(
                    f"[cyan]Background processing complete - finalized log session for process {process_id} with status: {status}[/cyan]"
                )
                ...
                if (
                    ...
                    try:
                        ...
                        if hasattr(
                            self.service.notion_sync_service, "sync_session_to_notion"
                        ):
                            sync_success = (
                                self.service.notion_sync_service.sync_session_to_notion(
                                    workflow_logger.session_id
                ...
                del self.service.workflow_loggers[process_id]
                console.print(
                    f"[cyan]Workflow logger cleaned up for completed process {process_id}[/cyan]"
                )

            except Exception as e:
                logger.error(
                    f"Failed to finalize workflow logger for process {process_id}: {e}"
                )
    ...
    async def handle_research_plan_generated_step(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
    ...
    async def handle_research_plan_approved_step(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        if process_id and user_id:
            try:
                ...
                logger.info(
                    "Context saved successfully after research plan approval step"
                )
            except Exception as save_err:
                logger.error(
                    f"Failed to save context after research plan approval step: {save_err}"
                )
    ...
    async def handle_researching_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if not context.research_plan or not hasattr(context.research_plan, "queries"):
            await self.service.utils.send_error(
                context, "リサーチ計画がありません。リサーチをスキップします。"
            )
            context.current_step = "error"
            return
        ...
        try:
            ...
            total_queries = len(context.research_plan.queries)
            console.print(
                f"[cyan]🚀 {total_queries}件のリサーチクエリを並列実行します...[/cyan]"
            )
            ...
            async def execute_query_with_websocket_progress(query, query_index: int):
                ...
                try:
                    console.print(
                        f"🔍 クエリ {query_index + 1}/{total_queries}: {query.query if hasattr(query, 'query') else str(query)}"
                    )
                    ...
                    if isinstance(agent_output, ResearchQueryResult):
                        console.print(
                            f"[green]✅ クエリ {query_index + 1} が完了しました。[/green]"
                        )
                        return query_index, agent_output, True
                    else:
                        console.print(
                            f"[yellow]⚠️ クエリ {query_index + 1} で予期しないエージェント出力タイプを受け取りました: {type(agent_output)}[/yellow]"
                        )
                        return query_index, None, False

                except Exception as e:
                    console.print(
                        f"[red]❌ クエリ {query_index + 1} でエラーが発生: {e}[/red]"
                    )
                    logger.error(f"Error in research query {query_index + 1}: {e}")
                    return query_index, None, False
            ...
            if successful_queries == 0:
                ...
                context.executing_step = None
                await self.service.utils.send_error(
                    context, "All research queries failed"
                )
                return
            ...
            context.executing_step = None
            console.print(
                f"[green]{len(context.research_query_results)}件のリサーチが完了しました。統合ステップに進みます。[/green]"
            )
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.error(
                        f"Failed to save context after research completion: {save_err}"
                    )
            ...
            context.executing_step = None
            await self.service.utils.send_error(
                context, f"リサーチ実行中にエラーが発生しました: {str(e)}"
            )
            context.current_step = "error"

    async def execute_research_synthesizing_background(
        self, context: "ArticleContext", run_config: RunConfig
    ):
        """リサーチ統合のバックグラウンド実行"""
        ...
        if isinstance(agent_output, ResearchReport):
            ...
            try:
                    ...
                    "create_process_event",
                    {
                        ...
                        "p_event_data": {
                            ...
                            "key_findings_count": len(
                                getattr(agent_output, "key_findings", [])
                            ),
                ...
                if result.data:
                    logger.info(
                        f"Published research_synthesis_completed event for process {getattr(context, 'process_id', 'unknown')}"
                    )

            except Exception as e:
                logger.error(
                    f"Error publishing research_synthesis_completed event: {e}"
                )
            ...
            if context.websocket:
                ...
                report_data = ResearchReportData(
                    topic=context.selected_theme.title
                ...
                await self.service.utils.send_server_event(
                    context, ResearchCompletePayload(report=report_data)
                )
        else:
            console.print(
                "[red]リサーチ合成中に予期しないエージェント出力タイプを受け取りました。[/red]"
            )
            context.current_step = "error"
    ...
    async def execute_research_background(
        ...
        if not context.selected_theme:
            console.print(
                "[red]テーマが選択されていません。リサーチをスキップします。[/red]"
            )
            context.current_step = "error"
            return
        ...
        if not context.selected_detailed_persona:
            console.print(
                "[red]詳細なペルソナが選択されていません。リサーチをスキップします。[/red]"
            )
            context.current_step = "error"
            return

        console.print(
            f"🔍 「{context.selected_theme.title}」について包括的なリサーチを開始します..."
        )
        ...
        try:
            agent_output = await self.run_agent(
                research_agent, agent_input, context, run_config
            )
            ...
            if isinstance(agent_output, ResearchReport):
                    ...
                    result = supabase.rpc(
                        "create_process_event",
                        {
                            ...
                            "p_event_data": {
                                ...
                                "report_summary": getattr(
                                    agent_output, "overall_summary", ""
                                ),
                                "key_points_count": len(
                                    getattr(agent_output, "key_points", [])
                                ),
                                "sources_count": len(
                                    getattr(agent_output, "all_sources", [])
                                ),
                                "timestamp": datetime.now(timezone.utc).isoformat(),
                            },
                            "p_event_category": "step_completion",
                ...
                if process_id and user_id:
                    try:
                        ...
                        logger.info(
                            "Context saved successfully after research completion"
                        )
                    except Exception as save_err:
                        logger.error(
                            f"Failed to save context after research completion: {save_err}"
                ...
                if is_outline_first:
                    ...
                    logger.info(
                        "Outline-first flow: Moving from research completion to writing_sections"
                    )
                else:
                    context.current_step = "outline_generating"
                    logger.info(
                        "Research-first flow: Moving from research completion to outline_generating"
                    )
                ...
                if context.websocket:
                        ...
                        for point in agent_output.key_points:
                            if hasattr(point, "point"):
                                key_points.append(
                                    KeyPointData(
                                        point=point.point,
                                        supporting_sources=getattr(
                                            point, "supporting_sources", []
                                        ),
                                    )
                                )
                            else:
                                key_points.append(
                                    KeyPointData(
                                        point=str(point), supporting_sources=[]
                                    )
                                )
                    ...
                    report_data = ResearchReportData(
                        ...
                        key_points=key_points,
                        interesting_angles=getattr(
                            agent_output, "interesting_angles", []
                        ),
                ...
                console.print(
                    f"[red]リサーチ中に予期しないエージェント出力タイプを受け取りました: {type(agent_output)}[/red]"
                )
                context.current_step = "error"
    ...
    async def execute_outline_generating_background(
        self, context: "ArticleContext", run_config: RunConfig
    ):
        """アウトライン生成のバックグラウンド実行"""
        ...
        if not is_outline_first and not context.research_report:
            console.print(
                "[red]リサーチ報告書がありません。アウトライン生成をスキップします。[/red]"
            )
            context.current_step = "error"
            return
        elif is_outline_first and not context.research_report:
            logger.info(
                "Outline-first flow: Generating outline without research report"
            )
        ...
        try:
            ...
            result = supabase.rpc(
                "create_process_event",
                {
                    ...
                    "p_event_data": {
                        ...
                        "theme_title": getattr(
                            context.selected_theme, "title", "Unknown"
                        ),
            ...
            if result.data:
                logger.info(
                    f"Published outline_generation_started event for process {getattr(context, 'process_id', 'unknown')}"
                )
        ...
        if is_outline_first and context.research_report is None:
            ...
            console.print(
                f"🤖 {current_agent.name} にアウトライン生成を依頼します (リサーチ前)..."
            )
        else:
            # リサーチ先行フローではリサーチ後にアウトライン生成
            if context.research_report is None:
                raise ValueError(
                    "Classic flow requires research_report before outline generation"
                )
            agent_input = f"テーマ: {context.selected_theme.title}\nペルソナ: {context.selected_detailed_persona}\nリサーチ報告書: {context.research_report.model_dump_json(indent=2)}\n目標文字数: {context.target_length}"
            console.print(
                f"🤖 {current_agent.name} にアウトライン生成を依頼します (リサーチ後)..."
            )
        ...
        if isinstance(agent_output, Outline):
            ...
            console.print(
                f"[cyan]アウトライン（{len(agent_output.sections)}セクション）を生成しました。[/cyan]"
            )
            ...
            if process_id and user_id and hasattr(self.service, "persistence_service"):
                try:
                    ...
                    logger.info(
                        f"✅ Context with generated outline saved to DB for process {process_id}"
                    )
                except Exception as save_err:
                    logger.error(
                        f"❌ Failed to save context after outline generation: {save_err}"
                    )
            else:
                logger.warning(
                    f"⚠️ Cannot save context - missing process_id: {process_id}, user_id: {user_id}, or persistence_service"
                )
            ...
            try:
                ...
                result = supabase.rpc(
                    "create_process_event",
                    {
                        ...
                        "p_event_data": {
                            ...
                            "suggested_tone": getattr(
                                agent_output, "suggested_tone", ""
                            ),
                            "timestamp": datetime.now(timezone.utc).isoformat(),
                        },
                        "p_event_category": "step_completion",
                ...
                if result.data:
                    logger.info(
                        f"Published outline_generation_completed event for process {getattr(context, 'process_id', 'unknown')}"
                    )

            except Exception as e:
                logger.error(
                    f"Error publishing outline_generation_completed event: {e}"
                )
        else:
            console.print(
                "[red]アウトライン生成中に予期しないエージェント出力タイプを受け取りました。[/red]"
            )
            context.current_step = "error"

    async def execute_writing_sections_background(
        self, context: "ArticleContext", run_config: RunConfig
    ):
        """セクション執筆のバックグラウンド実行"""
        if not context.generated_outline:
            console.print(
                "[red]承認されたアウトラインがありません。セクション執筆をスキップします。[/red]"
            )
            context.current_step = "error"
            return
        ...
        is_image_mode = getattr(context, "image_mode", False)
        console.print(
            f"[cyan]{'画像モード' if is_image_mode else '通常モード'}でセクションを執筆します。[/cyan]"
        )
        ...
        for i, section in enumerate(sections):
            ...
            try:
                ...
                if result.data:
                    logger.info(
                        f"Published section_writing_started event for section {i + 1} (background)"
                    )
            ...
            if is_image_mode:
                ...
                console.print(
                    f"[cyan]画像プレースホルダー対応エージェント ({current_agent.name}) を使用します。[/cyan]"
                )
            else:
                current_agent = section_writer_agent
                console.print(
                    f"[cyan]通常エージェント ({current_agent.name}) を使用します。[/cyan]"
                )
            ...
            if is_image_mode:
                user_request_parts.append(
                    "必要に応じて画像プレースホルダーを指定の形式で挿入してください。"
                )
            user_request = "\n".join(user_request_parts)
            current_input_messages: List[Dict[str, Any]] = list(
            ...
            if is_image_mode and isinstance(agent_output, ArticleSectionWithImages):
                ...
                console.print(
                    f"[green]セクション {i + 1} が完了しました（画像プレースホルダー {len(agent_output.image_placeholders)} 個含む）。[/green]"
                )
                ...
                article_section = ArticleSection(
                    title=section.heading, content=agent_output, order=i
                )
                context.generated_sections.append(article_section)
                section_content_length = len(agent_output)
                console.print(
                    f"[green]セクション {i + 1} が完了しました（HTML文字列形式）。[/green]"
                )

            else:
                console.print(
                    f"[red]セクション {i + 1} で予期しないエージェント出力タイプを受け取りました: {type(agent_output)}[/red]"
                )
                context.current_step = "error"
                return
            ...
            try:
                ...
                assistant_content = (
                    ...
                    else (agent_output if isinstance(agent_output, str) else "")
                )
            ...
            try:
                    ...
                    "create_process_event",
                    {
                        ...
                        "p_event_data": {
                            ...
                            "placeholders_count": len(
                                getattr(agent_output, "image_placeholders", [])
                            )
                ...
                if result.data:
                    logger.info(
                        f"Published section_completed event for section {i + 1} (background)"
                    )
        ...
        try:
                ...
                "create_process_event",
                {
                    ...
                    "p_event_data": {
                        ...
                        "total_content_length": sum(
                            len(getattr(s, "content", ""))
                            for s in context.generated_sections
                        ),
                        "total_placeholders": len(
                            getattr(context, "image_placeholders", [])
                        ),
            ...
            if result.data:
                logger.info(
                    f"Published all_sections_completed event for {total_sections} sections"
                )

        except Exception as e:
            logger.error(f"Error publishing all_sections_completed event: {e}")

    async def execute_editing_background(
        self,
        context: "ArticleContext",
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if not context.generated_sections_html:
            console.print(
                "[red]生成されたセクションがありません。編集をスキップします。[/red]"
            )
            context.current_step = "error"
            return
        ...
        combined_content = "\n\n".join(
            [
                section
                for section in context.generated_sections_html
        ...
        if context.websocket:
            await self.service.utils.send_server_event(
                context,
                EditingStartPayload(message="記事の最終編集を開始しています..."),
            )
        ...
        if isinstance(agent_output, RevisedArticle):
            ...
            await self.log_workflow_step(
                ...
                "completed",
                {
                    ...
                    "sections_count": len(context.generated_sections)
                    if hasattr(context, "generated_sections")
                    else 0,
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.error(
                        f"Failed to save context after article editing: {save_err}"
                    )
            ...
            await self.log_workflow_step(
                ...
                "completed",
                {
                    ...
                    "sections_count": len(context.generated_sections_html)
                    if hasattr(context, "generated_sections_html")
                    else 0,
                    "total_tokens_used": getattr(context, "total_tokens_used", 0),
                },
            )
            ...
            console.print(
                f"[red]編集中に予期しないエージェント出力タイプを受け取りました: {type(agent_output)}[/red]"
            )
            context.current_step = "error"
    ...
    async def handle_research_synthesizing_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        console.print(
            f"🤖 {current_agent.name} に詳細リサーチ結果の要約を依頼します..."
        )
        agent_output = await self.run_agent(
            current_agent, agent_input, context, run_config
        )
        ...
        if isinstance(agent_output, ResearchReport):
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.info(
                        "Context saved successfully after research report generation"
                    )
                except Exception as save_err:
                    logger.error(
                        f"Failed to save context after research report generation: {save_err}"
                    )
            ...
            await self.service.utils.send_error(
                context,
                f"リサーチ合成中に予期しないエージェント出力タイプ ({type(agent_output)}) を受け取りました。",
            )
            context.current_step = "error"

    async def handle_outline_generating_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if is_outline_first and context.research_report is None:
            ...
            console.print(
                f"🤖 {current_agent.name} にアウトライン作成を依頼します (リサーチ前)..."
            )
        else:
            if context.research_report is None:
                await self.service.utils.send_error(
                    context,
                    "リサーチレポートがありません。アウトライン作成をスキップします。",
                    "outline_generating",
                )
                context.current_step = "error"
                return
            ...
            research_report_json_str = json.dumps(
                context.research_report.model_dump(), indent=2
            )
            ...
            console.print(
                f"🤖 {current_agent.name} にアウトライン作成を依頼します (リサーチ後)..."
            )
        ...
        if isinstance(agent_output, Outline):
            ...
            context.current_step = "outline_generated"
            console.print(
                "[cyan]アウトラインを生成しました。クライアントの承認/編集/再生成を待ちます...[/cyan]"
            )
            ...
            await self.service.utils.send_error(
                context,
                f"アウトライン生成で確認が必要になりました: {agent_output.message}",
            )
            context.current_step = "error"
        else:
            await self.service.utils.send_error(
                context,
                f"アウトライン生成中に予期しないエージェント出力タイプ ({type(agent_output)}) を受け取りました。",
            )
            context.current_step = "error"

    async def handle_outline_generated_step(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        if context.generated_outline:
            ...
            if process_id and user_id:
                try:
                    ...
                    context.current_step = "outline_generated"
                    await self.service.persistence_service.update_process_state(
                        process_id=process_id, current_step_name="outline_generated"
                    )
                    ...
                    await supabase.rpc(
                        ...
                        {
                            "p_process_id": process_id,
                            "p_input_type": "approve_outline",
                            "p_timeout_minutes": 60,
                        },
                    ).execute()

                    logger.info(
                        "Process state marked waiting for outline approval with RPC"
                    )
                except Exception as save_err:
                    logger.error(
                        f"Failed to mark process waiting for outline approval: {save_err}"
                    )
            ...
            if user_response_message:
                ...
                if (
                    response_type == UserInputType.APPROVE_OUTLINE
                    and payload
                    and isinstance(payload, ApprovePayload)
                ):
                    if payload.approved:
                        context.current_step = "outline_approved"
                        console.print(
                            "[green]クライアントがアウトラインを承認しました。[/green]"
                        )
                        await self.service.utils.send_server_event(
                            context,
                            StatusUpdatePayload(
                        ...
                        if process_id and user_id:
                            try:
                                await (
                                    self.service.persistence_service.save_context_to_db(
                                        context, process_id=process_id, user_id=user_id
                                    )
                                )
                                logger.info(
                                    "Context saved successfully after outline approval"
                                )
                            except Exception as save_err:
                                logger.error(
                                    f"Failed to save context after outline approval: {save_err}"
                                )
                    else:
                        console.print(
                            "[yellow]クライアントがアウトラインを否認しました。再生成を試みます。[/yellow]"
                        )
                        context.current_step = "outline_generating"
                    ...
                    console.print(
                        "[yellow]クライアントがアウトラインの再生成を要求しました。[/yellow]"
                    )
                    context.current_step = "outline_generating"
                    context.generated_outline = None

                elif (
                    response_type == UserInputType.EDIT_OUTLINE
                    and payload
                    and isinstance(payload, EditOutlinePayload)
                ):
                    try:
                        edited_outline_data = payload.edited_outline
                        console.print(
                            "[green]アウトラインが編集されました（EditOutlinePayload）。[/green]"
                        )
                        # 編集されたアウトラインを適用
                        if isinstance(
                            edited_outline_data.get("title"), str
                        ) and isinstance(edited_outline_data.get("sections"), list):
                            top_level = edited_outline_data.get("top_level_heading")
                            if not isinstance(top_level, int) or top_level not in (
                                2,
                                3,
                            ):
                                top_level = getattr(
                                    context, "outline_top_level_heading", 2
                                )

                            normalized_outline = (
                                self.service.utils.normalize_outline_structure(
                                    {
                                        "title": edited_outline_data["title"],
                                        "suggested_tone": edited_outline_data.get(
                                            "suggested_tone", "丁寧で読みやすい解説調"
                                        ),
                                        "top_level_heading": top_level,
                                        "sections": edited_outline_data.get(
                                            "sections", []
                                        ),
                                    },
                            ...
                            context.generated_outline = normalized_outline
                            context.outline_top_level_heading = (
                                normalized_outline.top_level_heading
                            )
                            # CRITICAL: Set context.outline for research agent access
                            context.outline = normalized_outline
                            context.current_step = "outline_approved"
                            console.print(
                                "[green]編集されたアウトラインが適用されました（EditOutlinePayload）。[/green]"
                            )
                            await self.service.utils.send_server_event(
                                context,
                                StatusUpdatePayload(
                            ...
                            if process_id and user_id:
                                try:
                                    ...
                                    logger.info(
                                        "Context saved successfully after outline editing"
                                    )
                                except Exception as save_err:
                                    logger.error(
                                        f"Failed to save context after outline editing: {save_err}"
                                    )
                        else:
                            await self.service.utils.send_error(
                                context, "編集されたアウトラインの形式が無効です。"
                            )
                            context.current_step = "error"
                    except (ValidationError, TypeError, AttributeError) as e:
                        await self.service.utils.send_error(
                            context, f"アウトライン編集エラー: {e}"
                        )
                        context.current_step = "error"
                    ...
                    try:
                        ...
                        console.print(
                            "[green]アウトラインが編集されました（EditAndProceedPayload）。[/green]"
                        )
                        # 編集されたアウトラインを適用
                        if isinstance(
                            edited_outline_data.get("title"), str
                            ...
                            context.current_step = "outline_approved"
                            console.print(
                                "[green]編集されたアウトラインが適用されました（EditAndProceedPayload）。[/green]"
                            )
                            await self.service.utils.send_server_event(
                                context,
                                StatusUpdatePayload(
                    ...
                    try:
                        ...
                        console.print(
                            f"[yellow]EDIT_GENERIC received for outline step. Payload: {payload}[/yellow]"
                        )
                        if hasattr(payload, "edited_content"):
                            ...
                            if isinstance(
                                ...
                                context.current_step = "outline_approved"
                                console.print(
                                    "[green]編集されたアウトラインが適用されました（EDIT_GENERIC）。[/green]"
                                )
                                await self.service.utils.send_server_event(
                                    context,
                                    StatusUpdatePayload(
                                ...
                                await self.service.utils.send_error(
                                    context,
                                    "EDIT_GENERIC: 編集されたアウトラインの形式が無効です。",
                                )
                                context.current_step = "error"
                        else:
                            await self.service.utils.send_error(
                        ...
                        await self.service.utils.send_error(
                            context, f"EDIT_GENERIC アウトライン編集エラー: {e}"
                        )
                        context.current_step = "error"
                else:
                    await self.service.utils.send_error(
                        context, f"予期しない応答タイプ: {response_type}"
                    )
                    context.current_step = "error"
            else:
                console.print(
                    "[red]アウトラインの承認/編集でクライアントからの応答がありませんでした。[/red]"
                )
                context.current_step = "error"
        else:
            console.print(
                "[yellow]アウトラインが見つからないため生成ステップに戻します。[/yellow]"
            )
            context.current_step = "outline_generating"

    async def handle_outline_approved_step(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        if is_outline_first:
            ...
            console.print(
                f"[yellow]セクションライティング初期化: {len(context.generated_outline.sections)}セクションを実行予定[/yellow]"
            )
        ...
        if process_id and user_id:
            try:
                ...
                logger.error(
                    f"Failed to save context after outline approval step: {save_err}"
                )
    ...
    async def handle_writing_sections_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        if not context.generated_outline:
            await self.service.utils.send_error(
                context,
                "承認済みアウトラインがありません。セクション執筆をスキップします。",
            )
            context.current_step = "error"
            return
        ...
        # セクション完全性をチェック
        if self.service.utils.validate_section_completeness(
            context, context.generated_outline.sections, total_sections
        ):
            context.current_step = "editing"
            console.print(
                f"[green]全{total_sections}セクションの執筆が完了しました（{len(context.full_draft_html)}文字）。編集ステップに移ります。[/green]"
            )
        ...
        with safe_custom_span(
            ...
            if is_image_mode:
                ...
                console.print(
                    f"🤖 {current_agent.name} にセクション {target_index + 1} の執筆を依頼します (画像モード)..."
                )
                await self.service.utils.send_server_event(
                    context,
                    StatusUpdatePayload(
                ...
                if isinstance(agent_output, ArticleSectionWithImages):
                    ...
                    if (
                        agent_output.image_placeholders
                        and len(agent_output.image_placeholders) > 0
                    ):
                        console.print(
                            f"[cyan]セクション {target_index + 1} に画像プレースホルダーが含まれています: {len(agent_output.image_placeholders)}個[/cyan]"
                        )
                    else:
                        console.print(
                            f"[yellow]セクション {target_index + 1} には画像プレースホルダーが含まれていません（記事全体で1つ以上あれば問題ありません）[/yellow]"
                        )
                    ...
                    console.print(
                        f"[green]セクション {target_index + 1}「{generated_section.title}」を画像プレースホルダー付きで生成しました。（{len(agent_output.content)}文字、画像{len(agent_output.image_placeholders)}個）[/green]"
                    )
                    ...
                    await (
                        self.service.persistence_service.save_image_placeholders_to_db(
                            context, agent_output.image_placeholders, target_index
                        )
                    )

                    # セクション内容をcontextに保存
                    if len(context.generated_sections_html) <= target_index:
                        context.generated_sections_html.extend(
                            [""]
                            * (target_index + 1 - len(context.generated_sections_html))
                        )

                    context.generated_sections_html[target_index] = (
                        generated_section.content
                    )
                    context.last_agent_output = generated_section

                    # 会話履歴更新
                    last_user_request_item = (
                        agent_input[-1] if isinstance(agent_input, list) else None
                    )
                    if (
                        last_user_request_item
                        and last_user_request_item.get("role") == "user"
                    ):
                        user_request_text = last_user_request_item["content"][0]["text"]
                        context.add_to_section_writer_history("user", user_request_text)
                    context.add_to_section_writer_history(
                        "assistant", generated_section.content
                    )
                    ...
                    if process_id and user_id:
                        try:
                            ...
                            logger.info(
                                f"Context saved successfully after section {context.current_section_index}/{len(context.generated_outline.sections)} completion"
                            )
                        except Exception as save_err:
                            logger.error(
                                f"Failed to save context after section completion: {save_err}"
                            )

                    console.print(
                        f"[blue]セクション {target_index + 1} 完了。次のセクション: {context.current_section_index + 1}[/blue]"
                    )
                    ...
                    if context.websocket:
                        try:
                            ...
                            console.print(
                                f"[cyan]📤 Sending SectionChunkPayload for image mode: section_index={target_index}, heading='{target_heading}', is_image_mode=True, content_length={len(generated_section.content)}, placeholders={len(image_placeholders_data)}[/cyan]"
                            )
                            await self.service.utils.send_server_event(context, payload)
                            console.print(
                                f"[green]✅ SectionChunkPayload sent successfully for section {target_index}[/green]"
                            )
                        except Exception as e:
                            console.print(
                                f"[red]❌ Failed to send SectionChunkPayload for section {target_index}: {e}[/red]"
                            )
                    else:
                        console.print(
                            f"[yellow]⚠️ No WebSocket connection available for section {target_index} notification[/yellow]"
                        )
                else:
                    await self.service.utils.send_error(
                        context,
                        f"画像モードで予期しないAgent出力タイプ: {type(agent_output)}",
                    )
                    context.current_step = "error"
            else:
                # 通常モード: バッチ実行 (Converted from streaming to batch processing)
                console.print(
                    f"🤖 {current_agent.name} にセクション {target_index + 1} の執筆を依頼します (Batch)..."
                )
                await self.service.utils.send_server_event(
                    context,
                    StatusUpdatePayload(
                ...
                try:
                    ...
                    if result.data:
                        logger.info(
                            f"Published section_writing_started event for section {target_index + 1}"
                        )
                ...
                if isinstance(agent_output, str):
                    ...
                    generated_section = ArticleSection(
                        title=target_heading, content=agent_output, order=target_index
                    )
                elif hasattr(agent_output, "content") and hasattr(
                    agent_output, "title"
                ):
                    ...
                    console.print(
                        f"[red]予期しないエージェント出力タイプ: {type(agent_output)}[/red]"
                    )
                    generated_section = ArticleSection(
                        title=target_heading,
                ...
                try:
                    ...
                    result = supabase.rpc(
                        "create_process_event",
                        {
                            ...
                            "p_event_data": {
                                ...
                                "progress_percentage": int(
                                    (
                                        context.current_section_index
                                        / len(context.generated_outline.sections)
                                    )
                                    * 100
                                ),
                    ...
                    if result.data:
                        logger.info(
                            f"Published section_completed event for section {target_index + 1} (batch processing)"
                        )
                ...
                if context.websocket:
                    try:
                        ...
                        console.print(
                            f"[dim]WebSocket section completion event error (continuing): {ws_err}[/dim]"
                        )

    async def handle_editing_step(
        self,
        context: ArticleContext,
        run_config: RunConfig,
        process_id: Optional[str] = None,
        ...
        """編集ステップの処理"""
        if not context.generated_sections_html or not any(
            s.strip() for s in context.generated_sections_html
        ):
            await self.service.utils.send_error(
                context, "生成されたセクションがありません。編集をスキップします。"
            )
            context.current_step = "error"
            return
        ...
        await self.service.utils.send_server_event(
            context, EditingStartPayload(message="記事の最終編集を開始しています...")
        )
        ...
        if isinstance(agent_output, RevisedArticle):
            ...
            if process_id and user_id:
                try:
                    ...
                    logger.info(
                        "Context saved successfully after final article completion"
                    )
                except Exception as save_err:
                    logger.error(
                        f"Failed to save context after final article completion: {save_err}"
                    )
            ...
            if process_id and user_id:
                try:
                    ...
                    console.print(
                        f"[yellow]Warning: article_id の取得に失敗しました: {fetch_err}[/yellow]"
                    )
            ...
            await self.service.utils.send_error(
                context,
                f"編集中に予期しないAgent出力タイプ ({type(agent_output)}) を受け取りました。",
            )
            context.current_step = "error"

    async def handle_user_input_step(
        self,
        context: ArticleContext,
        process_id: Optional[str] = None,
        user_id: Optional[str] = None,
        ...
        console.print(
            f"[blue]ユーザー入力ステップを処理中: {context.current_step}[/blue]"
        )
        ...
        if context.current_step == "persona_generated":
            if context.generated_detailed_personas:
                ...
                user_response_message = await self.service.utils.request_user_input(
                    ...
                    UserInputType.SELECT_PERSONA,
                    GeneratedPersonasPayload(
                        personas=personas_data_for_client
                    ).model_dump(),
                )
                ...
                if user_response_message:
                    ...
                    if (
                        ...
                        if 0 <= selected_id < len(context.generated_detailed_personas):
                            context.selected_detailed_persona = (
                                context.generated_detailed_personas[selected_id]
                            )
                            context.current_step = "persona_selected"
                            console.print(
                                f"[green]ペルソナが選択されました: {context.selected_detailed_persona[:100]}...[/green]"
                            )
                            await self.service.utils.send_server_event(
                                context,
                                StatusUpdatePayload(
                            ...
                            if process_id and user_id:
                                try:
                                    ...
                                    logger.info(
                                        "Context saved successfully after persona selection"
                                    )
                                except Exception as save_err:
                                    logger.error(
                                        f"Failed to save context after persona selection: {save_err}"
                            ...
                            await self.service.utils.send_error(
                                context, f"無効なペルソナインデックス: {selected_id}"
                            )
                            context.current_step = "error"
                    elif response_type == UserInputType.REGENERATE:
                        console.print(
                            "[yellow]ペルソナの再生成が要求されました。[/yellow]"
                        )
                        context.current_step = "persona_generating"
                        context.generated_detailed_personas = []
                    elif (
                        ...
                        try:
                            ...
                            console.print(
                                f"[blue]EditAndProceedPayload received for persona: {edited_persona_data}[/blue]"
                            )
                            ...
                            if (
                                ...
                                context.current_step = "persona_selected"
                                console.print(
                                    f"[green]ペルソナが編集され選択されました: {description[:100]}...[/green]"
                                )
                                await self.service.utils.send_server_event(
                                    context,
                                    StatusUpdatePayload(
                                ...
                                if process_id and user_id:
                                    try:
                                        ...
                                        logger.info(
                                            "Context saved successfully after persona editing"
                                        )
                                    except Exception as save_err:
                                        logger.error(
                                            f"Failed to save context after persona editing: {save_err}"
                                ...
                                await self.service.utils.send_error(
                                    context,
                                    f"編集されたペルソナの形式が無効です。受信データ: {edited_persona_data}",
                                )
                                context.current_step = "error"
                        except (ValidationError, TypeError, AttributeError) as e:
                            await self.service.utils.send_error(
                                context, f"ペルソナ編集エラー: {e}"
                            )
                            context.current_step = "error"
                    else:
                        await self.service.utils.send_error(
                    ...
                    console.print(
                        "[red]ペルソナ選択でユーザー応答がありませんでした。[/red]"
                    )
                    context.current_step = "error"
            else:
                console.print(
                    "[yellow]ペルソナが見つからないため生成ステップに戻します。[/yellow]"
                )
                context.current_step = "persona_generating"
            ...
            if context.generated_themes:
                ...
                if user_response_message:
                    ...
                    if (
                        ...
                        selected_index = payload.selected_index
                        if 0 <= selected_index < len(context.generated_themes):
                            context.selected_theme = context.generated_themes[
                                selected_index
                            ]
                            context.current_step = "theme_selected"
                            console.print(
                                f"[green]テーマ「{context.selected_theme.title}」が選択されました。[/green]"
                            )
                            await self.service.utils.send_server_event(
                                context,
                                StatusUpdatePayload(
                            ...
                            if process_id and user_id:
                                try:
                                    ...
                                    logger.info(
                                        "Context saved successfully after theme selection"
                                    )
                                except Exception as save_err:
                                    logger.error(
                                        f"Failed to save context after theme selection: {save_err}"
                            ...
                            await self.service.utils.send_error(
                                context, f"無効なテーマインデックス: {selected_index}"
                            )
                            context.current_step = "error"
                    elif response_type == UserInputType.REGENERATE:
                        console.print(
                            "[yellow]テーマの再生成が要求されました。[/yellow]"
                        )
                        context.current_step = "theme_generating"
                        context.generated_themes = []
                    elif (
                        ...
                        try:
                            ...
                            if (
                                isinstance(edited_theme_data.get("title"), str)
                                and isinstance(
                                    edited_theme_data.get("description"), str
                                )
                                and isinstance(edited_theme_data.get("keywords"), list)
                            ):
                                from app.domains.seo_article.schemas import (
                                    ThemeProposalData as ThemeIdea,
                                )

                                context.selected_theme = ThemeIdea(**edited_theme_data)
                                context.current_step = "theme_selected"
                                console.print(
                                    f"[green]テーマが編集され選択されました: {context.selected_theme.title}[/green]"
                                )
                                await self.service.utils.send_server_event(
                                    context,
                                    StatusUpdatePayload(
                                ...
                                if process_id and user_id:
                                    try:
                                        ...
                                        logger.info(
                                            "Context saved successfully after theme editing"
                                        )
                                    except Exception as save_err:
                                        logger.error(
                                            f"Failed to save context after theme editing: {save_err}"
                                ...
                                await self.service.utils.send_error(
                                    context, "編集されたテーマの形式が無効です。"
                                )
                                context.current_step = "error"
                        except (ValidationError, TypeError, AttributeError) as e:
                            await self.service.utils.send_error(
                                context, f"テーマ編集エラー: {e}"
                            )
                            context.current_step = "error"
                    elif (
                        response_type == UserInputType.EDIT_THEME
                        and payload
                        and isinstance(payload, EditThemePayload)
                    ):
                        try:
                        ...
                        try:
                            ...
                            console.print(
                                f"[yellow]EDIT_GENERIC received for theme step. Payload: {payload}[/yellow]"
                            )
                            if hasattr(payload, "edited_content"):
                                edited_theme_data = payload.edited_content
                                if (
                                    ...
                                    and isinstance(
                                        edited_theme_data.get("keywords"), list
                                    )
                                ):
                                    from app.domains.seo_article.schemas import (
                                        ThemeProposalData as ThemeIdea,
                                    ...
                                    context.selected_theme = ThemeIdea(
                                        **edited_theme_data
                                    )
                                    context.current_step = "theme_selected"
                                    console.print(
                                        f"[green]テーマが編集され選択されました（EDIT_GENERIC）: {context.selected_theme.title}[/green]"
                                    )
                                    await self.service.utils.send_server_event(
                                        context,
                                        StatusUpdatePayload(
                            ...
                            await self.service.utils.send_error(
                                context, f"EDIT_GENERIC テーマ編集エラー: {e}"
                            )
                            context.current_step = "error"
                    else:
                        await self.service.utils.send_error(
                    ...
                    console.print(
                        "[red]テーマ選択でユーザー応答がありませんでした。[/red]"
                    )
                    context.current_step = "error"
            else:
                console.print(
                    "[yellow]テーマが見つからないため生成ステップに戻します。[/yellow]"
                )
                context.current_step = "theme_generating"
            ...
            if context.research_plan:
                from app.domains.seo_article.schemas import (
                    ResearchPlanData,
                    ResearchPlanQueryData,
                )

                plan_data = ResearchPlanData(
                    topic=context.research_plan.topic,
                    queries=[
                        ResearchPlanQueryData(query=q.query, focus=q.focus)
                ...
                if process_id and user_id:
                    try:
                        ...
                        await supabase.rpc(
                            ...
                            {
                                "p_process_id": process_id,
                                "p_input_type": "approve_plan",
                                "p_timeout_minutes": 60,
                            },
                        ).execute()

                        logger.info(
                            "Process state marked waiting for research plan approval with RPC"
                        )
                    except Exception as save_err:
                        logger.error(
                            f"Failed to mark process waiting for research plan approval: {save_err}"
                        )
                ...
                if user_response_message:
                    ...
                    if (
                        response_type == UserInputType.APPROVE_PLAN
                        and payload
                        and isinstance(payload, ApprovePayload)
                    ):
                        if payload.approved:
                            ...
                            if process_id and user_id:
                                try:
                                    ...
                                    logger.info(
                                        "Context saved successfully after research plan approval"
                                    )
                                except Exception as save_err:
                                    logger.error(
                                        f"Failed to save context after research plan approval: {save_err}"
                                    )
                        else:
                            console.print(
                                "[yellow]リサーチプランが否認されました。再生成を試みます。[/yellow]"
                            )
                            context.current_step = "research_planning"
                            context.research_plan = None
                    elif response_type == UserInputType.REGENERATE:
                        console.print(
                            "[yellow]リサーチプランの再生成が要求されました。[/yellow]"
                        )
                        context.current_step = "research_planning"
                        context.research_plan = None
                    elif (
                        response_type == UserInputType.EDIT_PLAN
                        and payload
                        and isinstance(payload, EditPlanPayload)
                    ):
                        try:
                            edited_plan_data = payload.edited_plan
                            if isinstance(
                                edited_plan_data.get("topic"), str
                            ) and isinstance(edited_plan_data.get("queries"), list):
                                from app.domains.seo_article.schemas import (
                                    ResearchPlan,
                                    ResearchQuery,
                                )
                                ...
                                queries = [
                                    ...
                                    for q in edited_plan_data["queries"]
                                ...
                                context.current_step = "research_plan_approved"
                                console.print(
                                    "[green]リサーチプランが編集され承認されました。[/green]"
                                )
                                await self.service.utils.send_server_event(
                                    context,
                                    StatusUpdatePayload(
                                ...
                                if process_id and user_id:
                                    try:
                                        ...
                                        logger.info(
                                            "Context saved successfully after research plan editing"
                                        )
                                    except Exception as save_err:
                                        logger.error(
                                            f"Failed to save context after research plan editing: {save_err}"
                                        )
                            else:
                                await self.service.utils.send_error(
                                    context,
                                    "編集されたリサーチプランの形式が無効です。",
                                )
                                context.current_step = "error"
                        except (ValidationError, TypeError, AttributeError) as e:
                            await self.service.utils.send_error(
                                context, f"リサーチプラン編集エラー: {e}"
                            )
                            context.current_step = "error"
                    elif (
                        response_type == UserInputType.EDIT_AND_PROCEED
                        ...
                        try:
                            ...
                            if isinstance(
                                ...
                                context.current_step = "research_plan_approved"
                                console.print(
                                    "[green]リサーチプランが編集され承認されました（EditAndProceedPayload）。[/green]"
                                )
                                await self.service.utils.send_server_event(
                                    context,
                                    StatusUpdatePayload(
                        ...
                        try:
                            ...
                            console.print(
                                f"[yellow]EDIT_GENERIC received for research plan step. Payload: {payload}[/yellow]"
                            )
                            if hasattr(payload, "edited_content"):
                                edited_plan_data = payload.edited_content
                                if isinstance(
                                    ...
                                    context.current_step = "research_plan_approved"
                                    console.print(
                                        "[green]リサーチプランが編集され承認されました（EDIT_GENERIC）。[/green]"
                                    )
                                    await self.service.utils.send_server_event(
                                        context,
                                        StatusUpdatePayload(
                                    ...
                                    await self.service.utils.send_error(
                                        context,
                                        "EDIT_GENERIC: 編集されたリサーチプランの形式が無効です。",
                                    )
                                    context.current_step = "error"
                            else:
                                await self.service.utils.send_error(
                            ...
                            await self.service.utils.send_error(
                                context, f"EDIT_GENERIC リサーチプラン編集エラー: {e}"
                            )
                            context.current_step = "error"
                    else:
                        await self.service.utils.send_error(
                    ...
                    console.print(
                        "[red]リサーチプラン承認でユーザー応答がありませんでした。[/red]"
                    )
                    context.current_step = "error"
            else:
                console.print(
                    "[yellow]リサーチプランが見つからないため生成ステップに戻します。[/yellow]"
                )
                context.current_step = "research_planning"

        else:
            console.print(
                f"[red]未実装のユーザー入力ステップ: {context.current_step}[/red]"
            )
            context.current_step = "error"
    ...
    async def execute_persona_generation_step(self, context: ArticleContext):
        ...
        try:
            ...
            if isinstance(agent_output, GeneratedPersonasResponse):
                ...
                context.current_step = "persona_generated"
                logger.info(
                    f"{len(context.generated_detailed_personas)}件の具体的なペルソナを生成しました。ユーザー選択待ちです。"
                )
            else:
                logger.error(
                    "ペルソナ生成中に予期しないエージェント出力タイプを受け取りました。"
                )
                context.current_step = "error"
    ...
    async def execute_theme_generation_step(self, context: ArticleContext):
        ...
        try:
            ...
            if isinstance(agent_output, ThemeProposal):
                ...
                context.current_step = "theme_proposed"
                logger.info(
                    f"{len(context.generated_themes)}件のテーマ案を生成しました。ユーザー選択待ちです。"
                )
            else:
                logger.error(
                    "テーマ生成中に予期しないエージェント出力タイプを受け取りました。"
                )
                context.current_step = "error"
    ...
    async def execute_single_research_query(
        self, context: ArticleContext, query, query_index: int
    ):
        ...
        try:
            ...
            if isinstance(agent_output, ResearchQueryResult):
                ...
                logger.warning(
                    f"Unexpected agent output type for research query {query_index + 1}: {type(agent_output)}"
                )
    ...
    async def write_single_section(
        self, context: ArticleContext, section, section_index: int
    ) -> str:
        ...
        try:
            ...
            try:
                ...
                if hasattr(agent_output, "content") and agent_output.content:
                    context.add_to_section_writer_history(
                        "assistant", agent_output.content
                    )
    ...
    async def save_step_snapshot_if_applicable(
        ...
        try:
            ...
            if completed_step not in SNAPSHOTABLE_STEPS:
                logger.debug(
                    f"⏭️ Skipping snapshot for non-snapshotable step: {completed_step}"
                )
                return
            ...
            if snapshot_id:
                logger.info(
                    f"📸 Snapshot saved for step '{completed_step}' (snapshot_id: {snapshot_id})"
                )
            else:
