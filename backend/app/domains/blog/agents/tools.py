...
async def memory_search(
    ...
    include_roles: Optional[List[Literal[
        "user_input", "qa", "assistant_output", "source", "system_note", "final_summary", "tool_result"
    ]]] = None,
    ...
    - `include_roles` で指定できる role:
      ...
      - `qa`: 追加質問フェーズのQ/A回答
      - `source`: 外部調査や参照情報の要点
      - `system_note`: 判断方針・制約・トンマナなどの内部メモ
    ...
    try:
        ...
        hits = await memory_service.search(
            ...
            per_process_item_limit=per_process_item_limit,
        )
