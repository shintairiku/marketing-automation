...
class SerpAPIService:
    ...
    async def analyze_keywords(self, keywords: List[str], num_articles_to_scrape: int = 5) -> SerpAnalysisResult:
        ...
        if scraped_articles:
            ...
            suggested_target_length = int(average_char_count * 1.1)  # 平均の1.1倍を推奨目標とする
        else:
    ...
    async def _get_search_results(self, query: str) -> Dict[str, Any]:
        ...
        """
        # 開発中はモックデータを優先的に使用
        print(f"モックデータを使用: {query}")
        return self._get_mock_search_results(query)
        
        # 実際のSerpAPI呼び出し（開発完了後に有効化）
        # try:
    ...
    async def _scrape_articles(self, search_results: Dict[str, Any], num_articles: int) -> List[ScrapedArticle]:
        ...
        """
        # 開発中は完全にモックデータを使用
        print(f"モックデータでスクレイピング: {num_articles}記事")
        return self._get_mock_scraped_articles(search_results, num_articles)
        
        # 実際のスクレイピング処理（ここを実装してください）
        # scraped_articles = []
        # 
    ...
    def _get_mock_scraped_articles(self, search_results: Dict[str, Any], num_articles: int) -> List[ScrapedArticle]:
        ...
        for i, question_data in enumerate(search_results.get("related_questions", [])[:2]):
            # 現実的な文字数を設定（2800-3000文字）
            target_char_count = 2800 + i * 200
            # 文字数に合わせてcontentを生成（約50文字の文章を必要回数繰り返す）
            base_text = f"この記事は{question_data.get('question', '質問')}について詳しく解説しています。自然素材を使った住宅は健康に良いとされています。"
            repeat_count = target_char_count // len(base_text)
            ...
            mock_articles.append(ScrapedArticle(
                ...
                content=content,
                char_count=len(content),  # 実際の文字数を設定
                image_count=2 + i * 2,
                source_type="related_question",
                question=question_data.get("question")
            ))
        ...
        for i, result in enumerate(search_results.get("organic_results", [])[:num_articles-len(mock_articles)]):
            # 現実的な文字数を設定（3000-3400文字）
            target_char_count = 3000 + i * 100
            # 文字数に合わせてcontentを生成
            base_text = f"この記事は{result.get('title', '記事')}について包括的に説明しています。注文住宅における自然素材の選び方や子育てに優しい間取りについて詳しく解説します。"
            repeat_count = target_char_count // len(base_text)
            content = base_text * repeat_count
