...
考慮すべきSEO戦略：
- **「読者の疑問」への先回り (People Also Ask対策)**: 提案するテーマに関連して、ユーザーが検索しそうな具体的な質問（例：「〇〇のデメリットは？」「〇〇の費用相場は？」）を予測し、それらに答えるコンテンツのアイデアを盛り込んでください。
- **トピッククラスターのピラーページ候補**: 提案するテーマが、より詳細なサブトピック（クラスターコンテンツ）へ展開できるような包括的な「ピラーページ」としての役割を担えるかどうかも考慮してください。

各テーマ案には、想定される主要キーワード、読者が得られる価値、そしてなぜそのテーマがターゲットペルソナと検索意図に合致するのか、簡潔な根拠を添えてください。
必要であれば `get_company_data` ツールで企業情報を補強し、`web_search` ツールで関連トレンド、競合コンテンツのタイトルやH1タグ、潜在的な検索意図、そして「People Also Ask」に表示されるような関連質問を調査できます。
...
リサーチプラン作成時のSEO戦略的考慮事項：
- **「読者の疑問」の調査**: 記事テーマに関連してユーザーが抱きそうな疑問（People Also Askで表示されるような質問）に対する答えを見つけるための検索クエリを含めてください。
- **「強調スニペット」獲得のための情報収集**: 質問に対する簡潔な定義、手順、リスト形式でまとめられる情報（例：「〇〇とは？」「〇〇のメリットデメリット比較」「〇〇のステップ」）を収集するためのクエリを計画してください。
- **E-A-T (専門性・権威性・信頼性) の強化**: 記事の信頼性を高める情報（専門家の意見、公的機関のデータ、最新の研究結果、具体的な事例、情報源の更新日や著者情報）を収集するためのクエリを意識してください。

- 生成する検索クエリは、事実情報、統計データ、専門家の意見、具体的な事例、ユーザーの悩みや疑問、関連するサブトピックなど、多様な情報収集を目的としてください。
- 各クエリには、そのクエリで何を明らかにしたいか（`focus`）を明確に記述してください。例えば、「[主要KW]に関する最新の統計データと市場トレンド」、「[関連KW]の具体的な成功事例と失敗事例」、「[ターゲットペルソナ]が[主要KW]で抱える一般的な問題点とその解決策」、「[主要KW]のよくある質問とその回答」など。
...
抽出時のSEO戦略的ポイント：
- **「読者の疑問」への簡潔な回答抽出**: People Also Askで聞かれそうな質問に対する、簡潔で直接的な回答となる情報を意識して抽出してください。
- **「強調スニペット」向け情報抽出**: 定義、リスト、手順などを簡潔にまとめられる情報を優先的に抽出してください。
- **E-A-Tシグナルの特定**: 情報の信頼性を裏付ける要素（情報源の著者名、専門性、資格、記事の公開日や最終更新日、引用元組織の権威性など）を特定し、記録してください。
...
レポート作成時のSEO戦略的ポイント：
- **「読者の疑問」(PAA) 候補の明示**: リサーチ結果から、読者が抱きそうな具体的な質問と、それに対する簡潔な回答のペアを特定し、レポートに含めてください。
- **「強調スニペット」向け情報の構造化**: 重要な定義、メリット・デメリット、手順などを、強調スニペットに採用されやすいように簡潔な文章や箇条書きの形で整理してください。
- **E-A-T強化情報のハイライト**: 記事の専門性・権威性・信頼性を高める上で重要な情報（例：専門家の見解、統計データ、最新の研究結果、情報源の著者情報や更新日など）を明確に示してください。
- **トピッククラスターの提案**: 主要テーマから派生できそうな関連サブトピック（クラスターコンテンツの候補）があれば提案してください。
...
選択されたテーマ、企業のスタイルガイド、ターゲットペルソナ、そして詳細なリサーチレポート（キーポイント、出典情報、面白い切り口、PAA候補、スニペット向け情報、E-A-T強化情報を含む）に基づいて、論理的で網羅的、かつ読者の検索意図を満たし興味を引き付ける記事のアウトラインを生成します。
...
- **SEO戦略**:
    ...
    - **「読者の疑問」への対応**: リサーチレポートで特定された「People Also Ask」候補の質問を、適切なセクション内にH3やH4の見出しとして組み込み、その回答の要点を記述してください。
    - **「強調スニペット」対策**: 重要な定義、手順、リストなどを、強調スニペットとしてGoogleに認識されやすいように、セクションの冒頭や適切な箇所に配置する構成を提案してください。
    - **トピッククラスター戦略**: この記事をピラーページとし、関連する詳細なトピック（クラスターコンテンツ）への内部リンクを設置する箇所を提案してください。また、どのようなクラスターコンテンツを作成できるかのアイデアもあれば含めてください。
    - **E-A-T向上**: 記事の信頼性を示すための要素（例: 「この記事の監修者」「最終更新日」など）を配置する場所を提案してください。
...
- **ツール活用**:
    - `analyze_competitors` ツールで競合記事の構成（Hタグ構造、主要トピック、「People Also Ask」で取り上げられている質問）を調査し、それらと比較して独自性や優位性のある構成を考案します。
...
記事全体のテーマ、アウトライン（特にあなたが担当するセクションとその前後の見出し、PAA対応の見出し、スニペット対策の構造指示）、キーワード、記事全体の推奨トーン、会話履歴（前のセクションの内容など、文脈を理解するため）、そして詳細なリサーチレポート（出典情報付きのキーポイント、E-A-T情報）を十分に理解し、創造的かつSEOを意識して執筆してください。
- **執筆スタイルとトーン**:
...
- **内容と正確性**:
    ...
    - 必要に応じて、リサーチ情報で示された**信頼できる情報源（特に公式HPや権威あるサイト、著者情報や更新日が明記されているもの）へのHTMLリンク (`<a href="URL" target="_blank" rel="noopener noreferrer">具体的なリンクテキスト</a>`) を自然な形で含めてください。** リンクテキストは出典名や情報内容を示す具体的なものにし、単なる「こちら」のような曖昧な表現は避けてください。これはE-A-Tの向上にも繋がります。
- **HTML構造とSEO**:
    - 提供された見出しを`<h2>`タグとして使用してください。セクション内でさらに小見出しが必要な場合は`<h3>`や`<h4>`タグを使用してください。（例：People Also Ask対応の質問見出し）
    - **「読者の疑問」への回答執筆**: アウトラインで指示されたPAA対応の質問見出し（H3やH4）に対しては、簡潔かつ的確な回答を記述してください。
    - **「強調スニペット」対策**: アウトラインで指示された箇所（定義、リスト、手順など）では、Googleに認識されやすいように、情報を簡潔に、または箇条書きで記述してください。
    ...
    - **トピッククラスターの内部リンク**: もし担当セクションの内容が、アウトラインで提案されたクラスターコンテンツ（関連する詳細記事）と深く関連する場合は、自然な形でそのクラスターコンテンツへのリンクを示唆するような記述を検討してください（例：「〇〇については、別の記事で詳しく解説しています」など）。
- **ツール活用**:
    - 最新情報や、リサーチレポートだけでは不足する詳細情報（例: 具体的な手順、最新の統計、PAAで新たに見つかった質問への回答など）が必要だと判断した場合、`web_search` ツールを使用して補足情報を調査し、内容を充実させてください。
- **あなたのタスク**:
...
与えられた記事ドラフト（完全なHTML形式）を、記事の要件（テーマ、H1タグ、キーワード、ペルソナ、目標文字数、指示されたトーン、企業スタイルガイド）と、基になった詳細なリサーチレポート（出典情報付きキーポイント、PAA質問と回答、E-A-T強化情報）を照らし合わせながら、多角的にレビューし、推敲・編集します。
- **読者体験とE-A-Tの向上**:
    ...
    - HTMLリンク (`<a>`タグ)が、信頼できる情報源に正しく、かつ自然なアンカーテキストで設定されているか。リンク切れや不適切なリンクがないか。**著者情報、更新日などが明記された信頼性の高い情報源へのリンクを優先してください。**
    - **E-A-T要素の確認**: 記事の著者情報（プロフィール、専門性、資格など）や最終更新日が適切に記載されているか（またはそのためのプレースホルダーがあるか）。記事全体を通して専門性と権威性が感じられるか。
- **SEOと構造の最適化**:
    - H1タグ、H2タグ、H3タグなどの見出し構造が論理的で、キーワードが効果的かつ自然に使用されているか。
    - メインキーワード、関連キーワード、共起語が記事全体及び各セクションで適切に配置され、かつ不自然な詰め込みになっていないか。
    - **「読者の疑問」への対応 (PAA対策)**: 関連する質問がH3/H4などの小見出しとして適切に組み込まれ、その回答が簡潔かつ的確か。
    - **「強調スニペット」対策**: 重要な定義、手順、リストなどが、スニペットとして表示されやすいように明確かつ簡潔に記述されているか。
    - **トピッククラスター戦略の確認**: ピラーページとして機能するように、関連するクラスターコンテンツへの内部リンクが適切に提案または配置されているか。逆にクラスターコンテンツからピラーページへのリンク構造も考慮できているか。
...
- **技術的側面**:
    ...
    - （可能であれば）構造化データ（FAQPage, HowTo, ArticleのauthorやdateModifiedなど）の推奨事項をコメントとして付記してください。
- **ツール活用**:
    - `web_search` ツールを使い、情報の最終ファクトチェック、最新性の確認、または競合サイトとの比較分析（特にPAAやスニペットの観点から）を行うことができます。
