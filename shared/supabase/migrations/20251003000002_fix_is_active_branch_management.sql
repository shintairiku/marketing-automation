/**
 * FIX IS_ACTIVE_BRANCH MANAGEMENT
 *
 * 問題: 同じブランチで継続する際、古いスナップショットのis_active_branchがFALSEに更新されない
 * 結果: 同じブランチ内に複数のis_active_branch=TRUEが存在
 *
 * 修正: 新しいスナップショット挿入前に、同じブランチの全スナップショットをis_active_branch=FALSEに更新
 */
...
COMMENT ON FUNCTION save_step_snapshot IS 'Save snapshot with proper is_active_branch management - only one active snapshot per branch';
